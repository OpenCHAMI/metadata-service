...
// This file provides the generated boilerplate for ClusterDefaults reconciler.
//
...
//
// To customize reconciliation logic, edit clusterdefaults_reconciler.go
package reconcilers
...
// ClusterDefaultsReconciler reconciles ClusterDefaults resources.
//
// This reconciler:
//   - Observes ClusterDefaults resources and updates their Status
...
//
// The implementation of reconcileClusterDefaults() is in clusterdefaults_reconciler.go
type ClusterDefaultsReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in clusterdefaults_reconciler.go
}

// NewDefaultClusterDefaultsReconciler creates a default ClusterDefaults reconciler.
//
...
//
// Returns:
//   - *ClusterDefaultsReconciler: Initialized reconciler
func NewDefaultClusterDefaultsReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *ClusterDefaultsReconciler {
	return &ClusterDefaultsReconciler{
...
func (r *ClusterDefaultsReconciler) GetResourceKind() string {
	return "ClusterDefaults"
}

// Reconcile brings ClusterDefaults to desired state.
//
// This method is called:
//   - When a ClusterDefaults resource is created/updated/deleted
...
//   - resource: The ClusterDefaults resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *ClusterDefaultsReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.ClusterDefaults // This is the typed struct
	...
	r.Logger.Debugf("Reconciling ClusterDefaults %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileClusterDefaults(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for ClusterDefaults %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for ClusterDefaults %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.clusterdefaultss.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for ClusterDefaults %s: %v", res.GetUID(), err)
