...
// This file provides the generated boilerplate for WireGuardPeer reconciler.
//
...
//
// To customize reconciliation logic, edit wireguardpeer_reconciler.go
package reconcilers
...
// WireGuardPeerReconciler reconciles WireGuardPeer resources.
//
// This reconciler:
//   - Observes WireGuardPeer resources and updates their Status
...
//
// The implementation of reconcileWireGuardPeer() is in wireguardpeer_reconciler.go
type WireGuardPeerReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in wireguardpeer_reconciler.go
}

// NewDefaultWireGuardPeerReconciler creates a default WireGuardPeer reconciler.
//
...
//
// Returns:
//   - *WireGuardPeerReconciler: Initialized reconciler
func NewDefaultWireGuardPeerReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *WireGuardPeerReconciler {
...
// Reconcile brings WireGuardPeer to desired state.
//
// This method is called:
//   - When a WireGuardPeer resource is created/updated/deleted
...
//   - resource: The WireGuardPeer resource to reconcile
//
// Returns:
...
func (r *WireGuardPeerReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.WireGuardPeer // This is the typed struct
	...
	r.Logger.Debugf("Reconciling WireGuardPeer %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileWireGuardPeer(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for WireGuardPeer %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for WireGuardPeer %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.wireguardpeers.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for WireGuardPeer %s: %v", res.GetUID(), err)
