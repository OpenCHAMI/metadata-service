...
// This file provides the generated boilerplate for Profile reconciler.
//
...
//
// To customize reconciliation logic, edit profile_reconciler.go
package reconcilers
...
// ProfileReconciler reconciles Profile resources.
//
// This reconciler:
//   - Observes Profile resources and updates their Status
...
//
// The implementation of reconcileProfile() is in profile_reconciler.go
type ProfileReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in profile_reconciler.go
}

// NewDefaultProfileReconciler creates a default Profile reconciler.
//
...
//
// Returns:
//   - *ProfileReconciler: Initialized reconciler
func NewDefaultProfileReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *ProfileReconciler {
...
func (r *ProfileReconciler) GetResourceKind() string {
	return "Profile"
}

// Reconcile brings Profile to desired state.
//
// This method is called:
//   - When a Profile resource is created/updated/deleted
...
//   - resource: The Profile resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *ProfileReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.Profile // This is the typed struct
	...
	r.Logger.Debugf("Reconciling Profile %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileProfile(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for Profile %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for Profile %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.profiles.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for Profile %s: %v", res.GetUID(), err)
