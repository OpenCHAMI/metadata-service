...
// This file provides the generated boilerplate for InstanceInfo reconciler.
//
...
//
// To customize reconciliation logic, edit instanceinfo_reconciler.go
package reconcilers
...
// InstanceInfoReconciler reconciles InstanceInfo resources.
//
// This reconciler:
//   - Observes InstanceInfo resources and updates their Status
...
//
// The implementation of reconcileInstanceInfo() is in instanceinfo_reconciler.go
type InstanceInfoReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in instanceinfo_reconciler.go
}

// NewDefaultInstanceInfoReconciler creates a default InstanceInfo reconciler.
//
...
//
// Returns:
//   - *InstanceInfoReconciler: Initialized reconciler
func NewDefaultInstanceInfoReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *InstanceInfoReconciler {
	return &InstanceInfoReconciler{
...
func (r *InstanceInfoReconciler) GetResourceKind() string {
	return "InstanceInfo"
}

// Reconcile brings InstanceInfo to desired state.
//
// This method is called:
//   - When a InstanceInfo resource is created/updated/deleted
...
//   - resource: The InstanceInfo resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *InstanceInfoReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.InstanceInfo // This is the typed struct
	...
	r.Logger.Debugf("Reconciling InstanceInfo %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileInstanceInfo(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for InstanceInfo %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for InstanceInfo %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.instanceinfos.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for InstanceInfo %s: %v", res.GetUID(), err)
