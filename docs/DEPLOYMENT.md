...
Prometheus metrics are generated by Fabrica and exposed when runtime metrics are
enabled:
...
- `http://<host>:<port>/metrics` on the main service listener
- `http://<host>:<metrics_port>/metrics` on the dedicated metrics listener

Generated metrics include:

- `main_http_request_total{method, handler, code}`
- `main_http_request_duration_seconds{method, handler, code}`
- `main_http_requests_in_flight`
- Go runtime and process collector metrics
