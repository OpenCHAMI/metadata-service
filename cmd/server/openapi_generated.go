...
// registerProfilePaths registers OpenAPI paths for Profile resources
func registerProfilePaths(spec *openapi3.T) {
	// Generate schemas from Go types - NO ANNOTATIONS NEEDED
	resourceSchema, _ := openapi3gen.NewSchemaRefForValue(&v1.Profile{}, spec.Components.Schemas)
	spec.Components.Schemas["Profile"] = resourceSchema

	createReqSchema, _ := openapi3gen.NewSchemaRefForValue(&CreateProfileRequest{}, spec.Components.Schemas)
	spec.Components.Schemas["CreateProfileRequest"] = createReqSchema

	updateReqSchema, _ := openapi3gen.NewSchemaRefForValue(&UpdateProfileRequest{}, spec.Components.Schemas)
	spec.Components.Schemas["UpdateProfileRequest"] = updateReqSchema
	...
	listOp := openapi3.NewOperation()
	listOp.OperationID = "listProfiles"
	listOp.Summary = "List all Profile resources"
	listOp.Description = "Returns a list of all Profile resources in the inventory"
	listOp.Tags = []string{"Profile"}
	listOp.Responses = openapi3.NewResponses()
	arraySchema := openapi3.NewArraySchema()
	arraySchema.Items = &openapi3.SchemaRef{Ref: "#/components/schemas/Profile"}
	listOp.Responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful response").
			WithJSONSchemaRef(&openapi3.SchemaRef{Value: arraySchema}),
	...
	createOp := openapi3.NewOperation()
	createOp.OperationID = "createProfile"
	createOp.Summary = "Create a new Profile resource"
	createOp.Description = "Creates a new Profile resource with the provided specification"
	createOp.Tags = []string{"Profile"}
	createOp.RequestBody = &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithJSONSchemaRef(&openapi3.SchemaRef{
	...
	getOp := openapi3.NewOperation()
	getOp.OperationID = "getProfile"
	getOp.Summary = "Get a specific Profile resource"
	getOp.Description = "Returns details of a specific Profile resource by UID"
	getOp.Tags = []string{"Profile"}
	getOp.Responses = openapi3.NewResponses()
	getOp.Responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful response").
	...
	updateOp := openapi3.NewOperation()
	updateOp.OperationID = "updateProfile"
	updateOp.Summary = "Update a Profile resource"
	updateOp.Description = "Updates an existing Profile resource with new values"
	updateOp.Tags = []string{"Profile"}
	updateOp.RequestBody = &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithJSONSchemaRef(&openapi3.SchemaRef{
	...
	deleteOp := openapi3.NewOperation()
	deleteOp.OperationID = "deleteProfile"
	deleteOp.Summary = "Delete a Profile resource"
	deleteOp.Description = "Removes a Profile resource from the inventory"
	deleteOp.Tags = []string{"Profile"}
	deleteOp.Responses = openapi3.NewResponses()
	deleteOp.Responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Resource deleted successfully").
	...
	uidParam := openapi3.NewPathParameter("uid").
		WithDescription("Unique identifier of the Profile resource").
		WithRequired(true).
		WithSchema(openapi3.NewStringSchema())
	...
	// Add paths to spec
	spec.Paths.Set("/profiles", collectionPath)
	spec.Paths.Set("/profiles/{uid}", itemPath)
}
